<template>
  <div class="dashboard-friends">
    <h1>👥 Amigos</h1>
    <p class="subtitle">Conecta con otros usuarios de HobitGo</p>

    <div class="search-section">
      <div class="search-box">
        <font-awesome-icon icon="search" class="search-icon" />
        <input v-model="searchQuery" type="text" placeholder="Buscar usuarios por nombre..." @input="searchUsers" />
      </div>
      <div v-if="searchResults.length > 0" class="search-results">
        <div v-for="user in searchResults" :key="user.id" class="search-item">
          ...
          <div class="user-info" @click="router.push(`/dashboard/profile/${user.id}`)">
            ...
            <span class="user-bio">{{ user.bio || 'Sin biografía' }}</span>
          </div>
          <div class="user-actions">
            ...
            <span v-else-if="getFriendStatus(user.id) === 'pending_received'" class="status-badge received">Te envió solicitud</span>
            <span v-else-if="getFriendStatus(user.id) === 'accepted'" class="status-badge accepted">✅ Amigos</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="friends-grid">
        ...
        <div class="requests-list">
          <div v-for="req in pendingRequests" :key="req.id" class="request-item">
            ...
            <div class="user-info" @click="router.push(`/dashboard/profile/${req.sender_id}`)">
              ...
              <span class="user-bio">{{ req.profiles?.bio || 'Sin biografía' }}</span>
            </div>
      ...
      <div class="section-card">
        ...
        <div v-if="friends.length > 0" class="friends-list">
          <div v-for="friend in friends" :key="friend.id" class="friend-item">
            ...
            <div class="user-info" @click="router.push(`/dashboard/profile/${friend.id}`)">
              ...
              <span class="user-bio">{{ friend.bio || 'Sin biografía' }}</span>
            </div>
        ...
        <p v-else class="empty-text">No tienes amigos aún. ¡Busca usuarios para añadir!</p>
      </div>
    </div>
  </div>
</template>
...
function getFriendStatus(userId) {
  const f = friendships.value.find(f =>
    (f.sender_id === currentUser.value.id && f.receiver_id === userId) ||
    (f.receiver_id === currentUser.value.id && f.sender_id === userId)
  )
  if (!f) return null
  ...
  if (friendIds.length > 0) {
    ...
    if (profiles) {
      friends.value = profiles.map(p => ({
        ...p,
        friendshipId: accepted.find(f => f.sender_id === p.id || f.receiver_id === p.id)?.id
      }))
    }
  } else {
    friends.value = []
...
function goToChat(userId, username) {
  router.push({ path: '/dashboard/chat', query: { user: userId, username } })
}
...
.btn-cancel-req { background: none; border: none; color: var(--text-muted); cursor: pointer; font-size: 16px; padding: 2px; transition: color 0.2s; }
.btn-cancel-req:hover { color: #e53935; }
.btn-message { background: #2196f3; color: #fff; border: none; border-radius: 8px; padding: 8px 12px; cursor: pointer; font-size: 14px; transition: opacity 0.2s; }
.btn-message:hover { opacity: 0.9; }
.btn-remove { background: none; border: none; color: var(--text-muted); cursor: pointer; padding: 8px; font-size: 14px; transition: color 0.2s; }
