...
onMounted(async () => {
  ...
  // Try the leaderboard view first; fall back to building from activity_log + profiles
  const { data, error } = await supabase.from('leaderboard').select('*')

  if (!error && data) {
    leaderboard.value = data
  } else {
    // Fallback: build leaderboard manually
    const [actRes, profRes, goalsRes, hobbiesRes] = await Promise.all([
      supabase.from('activity_log').select('user_id, points'),
      supabase.from('profiles').select('id, username, avatar_url'),
      supabase.from('goals').select('user_id').eq('completed', true),
      supabase.from('hobbies').select('user_id'),
    ])
    ...
    if (profRes.data) {
      ...
      ;(goalsRes.data || []).forEach(g => {
        goalsMap[g.user_id] = (goalsMap[g.user_id] || 0) + 1
      })
      ;(hobbiesRes.data || []).forEach(h => {
        hobbiesMap[h.user_id] = (hobbiesMap[h.user_id] || 0) + 1
      })

      leaderboard.value = profRes.data
        .map(p => ({
          id: p.id,
          username: p.username || 'Usuario',
          avatar_url: p.avatar_url || null,
          total_points: pointsMap[p.id] || 0,
          goals_completed: goalsMap[p.id] || 0,
          hobbies_count: hobbiesMap[p.id] || 0,
        }))
