    ...
    <div v-else class="chat-container">
      <div class="chat-list">
        <div class="chat-item" :class="{ active: activeTab === 'community' }" @click="activeTab = 'community'">
          ...
          <div class="chat-info"><h4>Comunidad</h4><p>Canal público</p></div>
        </div>
      ...
      <div class="chat-room">
        <div class="chat-header">
          ...
          <button v-if="activeTab !== 'community'" class="btn-view-profile" @click="goToProfile(activeTab)">
            Ver perfil
          </button>
        </div>
...
function goToProfile(userId) {
  if (!userId) return
  if (userId === currentUser.value?.id) router.push('/dashboard/profile')
...
async function loadProfile() {
  ...
  if (data) {
    ...
    // Si viene con query params de usuario, abrir chat privado
    if (route.query.user && route.query.username) {
      openPrivate({ user_id: route.query.user, username: route.query.username, lastMessage: '' })
    }
  } else {
...
async function saveUsername() {
  ...
  if (error) { usernameError.value = error.message.includes('unique') ? 'Ese nombre ya está en uso' : 'Error al guardar' }
  else {
...
async function loadConversations() {
  ...
  for (const msg of data) {
    ...
    if (!convMap[otherId]) convMap[otherId] = { user_id: otherId, lastMessage: msg.content }
  }
...
async function sendMessage() {
  ...
  if (activeTab.value === 'community') {
    await supabase.from('community_messages').insert({ user_id: currentUser.value.id, username: profile.value.username, content })
  } else {
    await supabase.from('private_messages').insert({ sender_id: currentUser.value.id, receiver_id: activeTab.value, content })
  }
}
...
.btn-view-profile { background: var(--bg-hover); border: none; border-radius: 8px; padding: 8px 14px; font-size: 13px; color: var(--text-secondary); cursor: pointer; transition: all 0.2s; }
...
.send-btn { width: 44px; height: 44px; border-radius: 50%; background: #E08E6B; color: #fff; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.2s; flex-shrink: 0; }
