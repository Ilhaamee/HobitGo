<template>
  <div class="public-profile-page">
    ...
    <div v-else-if="!profile" class="not-found">
      ...
      <button class="btn-back" @click="router.back()">{{ t('back') }}</button>
    </div>
    ...
    <div v-else>
      ...
      <div class="profile-header">
        ...
        <div class="profile-info">
          ...
          <p class="bio">{{ profile.bio || t('no_bio') }}</p>
          <div class="profile-actions">
            <button class="btn-message" @click="goToChat">
              <font-awesome-icon icon="comments" /> {{ t('send_message') }}
            </button>
            <button v-if="friendStatus === null" class="btn-add" @click="sendRequest">
              <font-awesome-icon icon="plus" /> {{ t('add_friend_btn') }}
            </button>
            <span v-else-if="friendStatus === 'pending_sent'" class="status-badge pending">{{ t('request_sent') }}</span>
            <span v-else-if="friendStatus === 'pending_received'">
              <button class="btn-accept" @click="acceptRequest">✅ {{ t('accept') }}</button>
            </span>
            <span v-else-if="friendStatus === 'accepted'" class="status-badge accepted">{{ t('already_friends') }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-info"><span class="stat-value">{{ stats.goalsCompleted }}</span><span class="stat-label">{{ t('completed_challenges_label') }}</span></div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
          ...
          <div class="stat-info"><span class="stat-value">{{ stats.friends }}</span><span class="stat-label">{{ t('friends_title') }}</span></div>
        </div>
      </div>
      ...
      <div class="bottom-grid">
        <div class="section-card">
          ...
          <p v-else class="empty-text">{{ t('no_hobbies_yet') }}</p>
        </div>

        <div class="section-card">
          <h2>🏆 {{ t('completed_challenges_label') }}</h2>
          <div v-if="completedGoals.length > 0" class="goals-list">
            <div v-for="goal in completedGoals" :key="goal.id" class="goal-item">
              ...
              <div class="goal-info">
                ...
                <span class="goal-days">{{ goal.total_days }} {{ currentLang === 'en' ? 'days' : 'días' }}</span>
              </div>
            </div>
          </div>
          <p v-else class="empty-text">{{ t('no_goals_yet') }}</p>
        </div>

        <div class="section-card" v-if="mutualFriends.length > 0">
          <h2>👥 {{ t('mutual_friends') }} ({{ mutualFriends.length }})</h2>
...
async function loadProfile() {
  ...
  if (user && userId === user.id) { router.replace('/dashboard/profile'); return }
  const { data } = await supabase.from('profiles').select('*').eq('id', userId).single()
  if (!data) { loading.value = false; return }
  profile.value = data
  await Promise.all([loadStats(userId), loadHobbies(userId), loadGoals(userId), loadFriendStatus(userId), loadMutualFriends(userId)])
  loading.value = false
}
...
async function loadStats(userId) {
  ...
  stats.value = { goalsCompleted: goalsRes.count || 0, hobbies: hobbiesRes.count || 0, points: activityRes.data ? activityRes.data.reduce((sum, a) => sum + (a.points || 0), 0) : 0, friends: friendsRes.count || 0 }
}
...
async function loadFriendStatus(userId) {
  ...
  const { data } = await supabase.from('friendships').select('*').or(`and(sender_id.eq.${currentUser.value.id},receiver_id.eq.${userId}),and(sender_id.eq.${userId},receiver_id.eq.${currentUser.value.id})`).single()
...
async function loadMutualFriends(userId) {
  ...
  if (!myRes.data || !theirRes.data) return
  const myIds = myRes.data.map(f => f.sender_id === currentUser.value.id ? f.receiver_id : f.sender_id)
  const theirIds = theirRes.data.map(f => f.sender_id === userId ? f.receiver_id : f.sender_id)
  const mutualIds = myIds.filter(id => theirIds.includes(id) && id !== currentUser.value.id && id !== userId)
...
async function sendRequest() {
  const { data } = await supabase.from('friendships').insert({ sender_id: currentUser.value.id, receiver_id: profile.value.id, status: 'pending' }).select()
...
async function acceptRequest() {
  ...
  friendStatus.value = 'accepted'; stats.value.friends++
}

function goToChat() { router.push({ path: '/dashboard/chat', query: { user: profile.value.id, username: profile.value.username } }) }
...
.btn-message { display: flex; align-items: center; gap: 6px; background: #2196f3; color: #fff; border: none; border-radius: 8px; padding: 10px 18px; font-size: 14px; font-weight: 600; cursor: pointer; }
.btn-message:hover { opacity: 0.9; }
.btn-add { display: flex; align-items: center; gap: 6px; background: #E08E6B; color: #fff; border: none; border-radius: 8px; padding: 10px 18px; font-size: 14px; font-weight: 600; cursor: pointer; }
...
@media (max-width: 600px) { .profile-header { flex-direction: column; text-align: center; } .profile-actions { justify-content: center; } .bottom-grid { grid-template-columns: 1fr; } }
</style>